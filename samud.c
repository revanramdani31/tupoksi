...
// ============================================================
//  STACK ADT :  UNDO ACTIONS (simple string stack)
// ============================================================
typedef struct{
...
void tampilkanMenu(){
    ...
    puts("3. Perbarui Status Tugas");
    puts("4. Hapus Tugas/Subtugas");
    puts("5. Tambah Tugas ke Antrian Prioritas");
    puts("6. Kerjakan Tugas (Dequeue)");
    puts("7. Undo Kerjaan Terakhir");
    puts("8. Laporan Progres");
    puts("9. Simpan Proyek ke File");
    puts("10. Keluar (tanpa simpan)");
    printf("Pilihan: ");
}
    ...
    while(1){
        ...
        switch(choice){
            ...
            case 2:{
                ...
                if(!parent){puts("Tidak ditemukan!");break;}
            ...
            case 3:{
                ...
                printf("Status sekarang: %s\n",statusStr(t->status));
                puts("Set status: 0=TODO,1=IN_PROGRESS,2=DONE");
                int st; scanf("%d",&st); flushStdin();
                if(st<0||st>2){puts("Invalid");break;}
                t->status=(Status)st;
                if(st==DONE) list_push(&doneList,t->name);
            }break;
            case 4:{
                printf("Nama tugas yang dihapus: "); getLine(buf,MAX_NAME);
                // simple delete among children of found parent
                TreeNode* parent=NULL, *target=NULL;
                // BFS to locate parent & child index
                // queue for internal search
                ...
                int idx=0;
                while(idx<parent->child_count && parent->child[idx]!=target) idx++;
            ...
            case 9:{
                ...
                FILE* f=fopen(buf,"w"); if(!f){puts("Gagal!");break;}
                node_save(f,root,0);
                fclose(f);
                puts("Disimpan.");
            }break;
            case 10: puts("Keluar..."); exit(0);
